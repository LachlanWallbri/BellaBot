...
public class DefaultHttp2LocalFlowController implements Http2LocalFlowController {
    ...
    public boolean consumeBytes(Http2Stream http2Stream, int i) throws Http2Exception {
        ...
        if (http2Stream.mo3940id() == 0) {
            throw new UnsupportedOperationException("Returning bytes for the connection window is not supported");
        }
        return state(http2Stream).consumeBytes(i) | connectionState().consumeBytes(i);
    }
    ...
    private class DefaultState implements FlowState {
        ...
        public void incrementFlowControlWindows(int i) throws Http2Exception {
            if (i > 0 && this.window > Integer.MAX_VALUE - i) {
                throw Http2Exception.streamError(this.stream.mo3940id(), Http2Error.FLOW_CONTROL_ERROR, "Flow control window overflowed for stream: %d", Integer.valueOf(this.stream.mo3940id()));
            }
            this.window += i;
            this.processedWindow += i;
            if (i >= 0) {
        ...
        public void receiveFlowControlledFrame(int i) throws Http2Exception {
            ...
            if (i2 < this.lowerBound) {
                throw Http2Exception.streamError(this.stream.mo3940id(), Http2Error.FLOW_CONTROL_ERROR, "Flow control window exceeded for stream: %d", Integer.valueOf(this.stream.mo3940id()));
            }
        }
        ...
        private void returnProcessedBytes(int i) throws Http2Exception {
            ...
            if (i2 - i < this.window) {
                throw Http2Exception.streamError(this.stream.mo3940id(), Http2Error.INTERNAL_ERROR, "Attempting to return too many bytes for stream %d", Integer.valueOf(this.stream.mo3940id()));
            }
        ...
        public boolean writeWindowUpdateIfNeeded() throws Http2Exception {
            ...
            if (!this.endOfStream && (i = this.initialStreamWindowSize) > 0) {
                if (this.processedWindow <= ((int) (i * this.streamWindowUpdateRatio))) {
                    writeWindowUpdate();
                    return true;
                }
            }
        ...
        private void writeWindowUpdate() throws Http2Exception {
            ...
            try {
                ...
                DefaultHttp2LocalFlowController.this.frameWriter.writeWindowUpdate(DefaultHttp2LocalFlowController.this.ctx, this.stream.mo3940id(), i, DefaultHttp2LocalFlowController.this.ctx.newPromise());
            } catch (Throwable th) {
                throw Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, th, "Attempting to return too many bytes for stream %d", Integer.valueOf(this.stream.mo3940id()));
            }
        }
    }
