...
public class DefaultHttp2Connection implements Http2Connection {
    ...
    public DefaultHttp2Connection(boolean z, int i) {
        ...
        this.streamMap.put(this.connectionStream.mo3940id(), (int) this.connectionStream);
    }
        ...
        try {
            forEachActiveStream(new Http2StreamVisitor() { // from class: io.netty.handler.codec.http2.DefaultHttp2Connection.1
                ...
                public boolean visit(Http2Stream http2Stream) {
                    if (http2Stream.mo3940id() <= i || !DefaultHttp2Connection.this.localEndpoint.isValidStreamId(http2Stream.mo3940id())) {
                        return true;
                    }
                    http2Stream.close();
                    return true;
    ...
    public void goAwaySent(final int i, long j, ByteBuf byteBuf) {
        ...
        try {
            forEachActiveStream(new Http2StreamVisitor() { // from class: io.netty.handler.codec.http2.DefaultHttp2Connection.2
                @Override // io.netty.handler.codec.http2.Http2StreamVisitor
                public boolean visit(Http2Stream http2Stream) {
                    if (http2Stream.mo3940id() <= i || !DefaultHttp2Connection.this.remoteEndpoint.isValidStreamId(http2Stream.mo3940id())) {
                        return true;
                    }
                    http2Stream.close();
                    return true;
    ...
    void removeStream(DefaultStream defaultStream, Iterator<?> it) {
        boolean z = true;
        if (it == null) {
            if (this.streamMap.remove(defaultStream.mo3940id()) == null) {
                z = false;
            }
        } else {
            it.remove();
    ...
    /* loaded from: classes.dex */
    public static /* synthetic */ class C71913 {
    ...
    static Http2Stream.State activeState(int i, Http2Stream.State state, boolean z, boolean z2) throws Http2Exception {
        int i2 = C71913.$SwitchMap$io$netty$handler$codec$http2$Http2Stream$State[state.ordinal()];
        if (i2 == 1) {
    ...
    public class DefaultStream implements Http2Stream {
        ...
        public Http2Stream open(boolean z) throws Http2Exception {
            this.state = DefaultHttp2Connection.activeState(this.f8506id, this.state, isLocal(), z);
        ...
        public Http2Stream closeLocalSide() {
            int i = C71913.$SwitchMap$io$netty$handler$codec$http2$Http2Stream$State[this.state.ordinal()];
            if (i == 4) {
        ...
        DefaultEndpoint<? extends Http2FlowController> createdBy() {
            return DefaultHttp2Connection.this.localEndpoint.isValidStreamId(this.f8506id) ? DefaultHttp2Connection.this.localEndpoint : DefaultHttp2Connection.this.remoteEndpoint;
        }

        final boolean isLocal() {
            return DefaultHttp2Connection.this.localEndpoint.isValidStreamId(this.f8506id);
        }
    ...
    public final class DefaultEndpoint<F extends Http2FlowController> implements Http2Connection.Endpoint<F> {
        ...
        public DefaultStream reservePushStream(int i, Http2Stream http2Stream) throws Http2Exception {
            ...
            if (!isLocal() ? http2Stream.state().remoteSideOpen() : http2Stream.state().localSideOpen()) {
                throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Stream %d is not open for sending push promise", Integer.valueOf(http2Stream.mo3940id()));
            }
        ...
        private void addStream(DefaultStream defaultStream) {
            DefaultHttp2Connection.this.streamMap.put(defaultStream.mo3940id(), (int) defaultStream);
