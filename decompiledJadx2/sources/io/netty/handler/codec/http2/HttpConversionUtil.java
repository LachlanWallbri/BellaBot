...
public final class HttpConversionUtil {
    ...
    static {
        ...
        OUT_OF_MESSAGE_SEQUENCE_RETURN_CODE = HttpResponseStatus.f8500OK;
    ...
    private static CharSequenceMap<AsciiString> toLowercaseMap(Iterator<? extends CharSequence> it, int i) {
        ...
        while (it.hasNext()) {
            AsciiString lowerCase = AsciiString.m3944of(it.next()).toLowerCase();
            try {
    ...
    private static void toHttp2HeadersFilterTE(Map.Entry<CharSequence, CharSequence> entry, Http2Headers http2Headers) {
        if (AsciiString.indexOf(entry.getValue(), ',', 0) == -1) {
            if (AsciiString.contentEqualsIgnoreCase(AsciiString.trim(entry.getValue()), HttpHeaderValues.TRAILERS)) {
                http2Headers.add((Http2Headers) HttpHeaderNames.f8498TE, HttpHeaderValues.TRAILERS);
            }
        } else {
    ...
    public static void toHttp2Headers(HttpHeaders httpHeaders, Http2Headers http2Headers) {
        ...
        while (iteratorCharSequence.hasNext()) {
            ...
            AsciiString lowerCase = AsciiString.m3944of(next.getKey()).toLowerCase();
            if (!HTTP_TO_HTTP2_HEADER_BLACKLIST.contains(lowerCase) && !lowercaseMap.contains(lowerCase)) {
                if (lowerCase.contentEqualsIgnoreCase(HttpHeaderNames.f8498TE)) {
                    toHttp2HeadersFilterTE(next, http2Headers);
                } else if (lowerCase.contentEqualsIgnoreCase(HttpHeaderNames.COOKIE)) {
                    AsciiString m3944of = AsciiString.m3944of(next.getValue());
                    try {
                        int forEachByte = m3944of.forEachByte(ByteProcessor.FIND_SEMI_COLON);
                        if (forEachByte != -1) {
                            int i = 0;
                            do {
                                http2Headers.add((Http2Headers) HttpHeaderNames.COOKIE, m3944of.subSequence(i, forEachByte, false));
                                i = forEachByte + 2;
                                if (i >= m3944of.length()) {
                                    break;
                                } else {
                                    forEachByte = m3944of.forEachByte(i, m3944of.length() - i, ByteProcessor.FIND_SEMI_COLON);
                                }
                            } while (forEachByte != -1);
                            if (i >= m3944of.length()) {
                                throw new IllegalArgumentException("cookie value is of unexpected format: " + ((Object) m3944of));
                            }
                            http2Headers.add((Http2Headers) HttpHeaderNames.COOKIE, m3944of.subSequence(i, m3944of.length(), false));
                        } else {
                            http2Headers.add((Http2Headers) HttpHeaderNames.COOKIE, m3944of);
                        }
                    } catch (Exception e) {
                        throw new IllegalStateException(e);
                    }
    ...
    private static void setHttp2Scheme(HttpHeaders httpHeaders, URI uri, Http2Headers http2Headers) {
        ...
        if (str != null) {
            http2Headers.scheme(AsciiString.m3944of(str));
    ...
    public static final class Http2ToHttpHeaderTranslator {
        ...
        static {
            CharSequenceMap<AsciiString> charSequenceMap = new CharSequenceMap<>();
            RESPONSE_HEADER_TRANSLATIONS = charSequenceMap;
            charSequenceMap.add((CharSequenceMap<AsciiString>) Http2Headers.PseudoHeaderName.AUTHORITY.value(), HttpHeaderNames.HOST);
        ...
        public void translate(Map.Entry<CharSequence, CharSequence> entry) throws Http2Exception {
            ...
            if (asciiString != null) {
                this.output.add(asciiString, AsciiString.m3944of(value));
                return;
            }
