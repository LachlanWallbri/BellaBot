...
public class JZlibEncoder extends ZlibEncoder {
    ...
    public JZlibEncoder(ZlibWrapper zlibWrapper, int i, int i2, int i3) {
        ...
        int init = this.f8478z.init(i, i2, i3, ZlibUtil.convertWrapperType(zlibWrapper));
        if (init != 0) {
            ZlibUtil.fail(this.f8478z, "initialization failure", init);
        }
    ...
    public JZlibEncoder(int i, int i2, int i3, byte[] bArr) {
        ...
        if (deflateInit != 0) {
            ZlibUtil.fail(this.f8478z, "initialization failure", deflateInit);
        } else {
            int deflateSetDictionary = this.f8478z.deflateSetDictionary(bArr, bArr.length);
            if (deflateSetDictionary != 0) {
                ZlibUtil.fail(this.f8478z, "failed to set the dictionary", deflateSetDictionary);
            }
        }
    ...
    public void encode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, ByteBuf byteBuf2) throws Exception {
        ...
        try {
            ...
            this.f8478z.avail_in = readableBytes;
            if (hasArray) {
                this.f8478z.next_in = byteBuf.array();
                this.f8478z.next_in_index = byteBuf.arrayOffset() + byteBuf.readerIndex();
            } else {
                byte[] bArr = new byte[readableBytes];
                byteBuf.getBytes(byteBuf.readerIndex(), bArr);
                this.f8478z.next_in = bArr;
                this.f8478z.next_in_index = 0;
            }
            int i = this.f8478z.next_in_index;
            int ceil = ((int) Math.ceil(readableBytes * 1.001d)) + 12 + this.wrapperOverhead;
            byteBuf2.ensureWritable(ceil);
            this.f8478z.avail_out = ceil;
            this.f8478z.next_out = byteBuf2.array();
            this.f8478z.next_out_index = byteBuf2.arrayOffset() + byteBuf2.writerIndex();
            int i2 = this.f8478z.next_out_index;
            try {
                int deflate = this.f8478z.deflate(2);
                if (deflate != 0) {
                    ZlibUtil.fail(this.f8478z, "compression failure", deflate);
                }
                int i3 = this.f8478z.next_out_index - i2;
                if (i3 > 0) {
                    byteBuf2.writerIndex(byteBuf2.writerIndex() + i3);
                }
            } finally {
                byteBuf.skipBytes(this.f8478z.next_in_index - i);
            }
        } finally {
            this.f8478z.next_in = null;
            this.f8478z.next_out = null;
        }
    }
    ...
    public ChannelFuture finishEncode(ChannelHandlerContext channelHandlerContext, ChannelPromise channelPromise) {
        ...
        this.finished = true;
        try {
            this.f8478z.next_in = EmptyArrays.EMPTY_BYTES;
            this.f8478z.next_in_index = 0;
            this.f8478z.avail_in = 0;
            byte[] bArr = new byte[32];
            this.f8478z.next_out = bArr;
            this.f8478z.next_out_index = 0;
            this.f8478z.avail_out = 32;
            int deflate = this.f8478z.deflate(4);
            if (deflate != 0 && deflate != 1) {
                channelPromise.setFailure((Throwable) ZlibUtil.deflaterException(this.f8478z, "compression failure", deflate));
                return channelPromise;
            }
            if (this.f8478z.next_out_index != 0) {
                byteBuf = Unpooled.wrappedBuffer(bArr, 0, this.f8478z.next_out_index);
            } else {
                byteBuf = Unpooled.EMPTY_BUFFER;
            }
            this.f8478z.deflateEnd();
            this.f8478z.next_in = null;
            this.f8478z.next_out = null;
