...
public abstract class HttpHeaders implements Iterable<Map.Entry<String, String>> {
    ...
    public static final class Names {
        ...
        /* renamed from: TE */
        public static final String f8499TE = "TE";
    ...
    private static boolean containsCommaSeparatedTrimmed(CharSequence charSequence, CharSequence charSequence2, boolean z) {
        if (z) {
            ...
            if (indexOf == -1) {
                ...
                int i = 0;
                while (!AsciiString.contentEqualsIgnoreCase(AsciiString.trim(charSequence.subSequence(i, indexOf)), charSequence2)) {
                    i = indexOf + 1;
                    indexOf = AsciiString.indexOf(charSequence, ',', i);
                    if (indexOf == -1) {
                        if (i < charSequence.length() && AsciiString.contentEqualsIgnoreCase(AsciiString.trim(charSequence.subSequence(i, charSequence.length())), charSequence2)) {
                            return true;
                        }
                    }
                }
            ...
            if (indexOf2 == -1) {
                ...
                int i2 = 0;
                while (!AsciiString.contentEquals(AsciiString.trim(charSequence.subSequence(i2, indexOf2)), charSequence2)) {
                    i2 = indexOf2 + 1;
                    indexOf2 = AsciiString.indexOf(charSequence, ',', i2);
                    if (indexOf2 == -1) {
                        if (i2 < charSequence.length() && AsciiString.contentEquals(AsciiString.trim(charSequence.subSequence(i2, charSequence.length())), charSequence2)) {
                            return true;
                        }
                    }
                }
