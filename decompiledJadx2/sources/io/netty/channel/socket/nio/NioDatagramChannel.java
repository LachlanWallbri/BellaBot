...
public final class NioDatagramChannel extends AbstractNioMessageChannel implements DatagramChannel {
    ...
    public boolean isActive() {
        java.nio.channels.DatagramChannel mo4506javaChannel = mo4506javaChannel();
        return mo4506javaChannel.isOpen() && ((((Boolean) this.config.getOption(ChannelOption.DATAGRAM_CHANNEL_ACTIVE_ON_REGISTRATION)).booleanValue() && isRegistered()) || mo4506javaChannel.socket().isBound());
    }

    @Override // io.netty.channel.socket.DatagramChannel
    public boolean isConnected() {
        return mo4506javaChannel().isConnected();
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // io.netty.channel.nio.AbstractNioChannel
    /* renamed from: javaChannel */
    public java.nio.channels.DatagramChannel mo4506javaChannel() {
        return (java.nio.channels.DatagramChannel) super.mo4506javaChannel();
    }
    ...
    private void doBind0(SocketAddress socketAddress) throws Exception {
        if (PlatformDependent.javaVersion() >= 7) {
            ...
            mo4506javaChannel().socket().bind(socketAddress);
        }
    }
    ...
    protected boolean doConnect(SocketAddress socketAddress, SocketAddress socketAddress2) throws Exception {
        ...
        try {
            mo4506javaChannel().connect(socketAddress);
            return true;
        } catch (Throwable th) {
            doClose();
            throw th;
    ...
    protected int doReadMessages(List<Object> list) throws Exception {
        ...
        try {
            ...
            InetSocketAddress inetSocketAddress = (InetSocketAddress) mo4506javaChannel.receive(internalNioBuffer);
    ...
    protected boolean doWriteMessage(Object obj, ChannelOutboundBuffer channelOutboundBuffer) throws Exception {
        ...
        if (socketAddress != null) {
            write = mo4506javaChannel().send(internalNioBuffer, socketAddress);
        } else {
            write = mo4506javaChannel().write(internalNioBuffer);
        }
        return write > 0;
    }
    ...
    public ChannelFuture joinGroup(InetAddress inetAddress, NetworkInterface networkInterface, InetAddress inetAddress2, ChannelPromise channelPromise) {
        ...
        try {
            if (inetAddress2 == null) {
                join = mo4506javaChannel().join(inetAddress, networkInterface);
            } else {
                join = mo4506javaChannel().join(inetAddress, networkInterface, inetAddress2);
            }
            synchronized (this) {
