...
public final class FlowableWindowBoundarySelector<T, B, V> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    public static final class WindowBoundaryMainSubscriber<T, B, V> extends QueueDrainSubscriber<T, Object, Flowable<T>> implements Subscription {
        ...
        /* renamed from: ws */
        final List<UnicastProcessor<T>> f8682ws;
        ...
        public void onNext(T t) {
            ...
            if (fastEnter()) {
                Iterator<UnicastProcessor<T>> it = this.f8682ws.iterator();
                while (it.hasNext()) {
                    it.next().onNext(t);
                }
                if (leave(-1) == 0) {
        ...
        void drainLoop() {
            ...
            List<UnicastProcessor<T>> list = this.f8682ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (!z2) {
                    if (poll instanceof WindowOperation) {
                        ...
                        if (windowOperation.f8683w != null) {
                            if (list.remove(windowOperation.f8683w)) {
                                windowOperation.f8683w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    dispose();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseSubscriber<T, V> operatorWindowBoundaryCloseSubscriber) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseSubscriber.f8681w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    public static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f8683w;

        WindowOperation(UnicastProcessor<T> unicastProcessor, B b) {
            this.f8683w = unicastProcessor;
            this.open = b;
        }
    }
    ...
    public static final class OperatorWindowBoundaryCloseSubscriber<T, V> extends DisposableSubscriber<V> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f8681w;
