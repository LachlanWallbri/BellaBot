...
public class GMSSPrivateKeyParameters extends GMSSKeyParameters {
    ...
    private GMSSPrivateKeyParameters(GMSSPrivateKeyParameters gMSSPrivateKeyParameters) {
        ...
        this.f9876K = gMSSPrivateKeyParameters.f9876K;
    ...
    public GMSSPrivateKeyParameters(int[] iArr, byte[][] bArr, byte[][] bArr2, byte[][][] bArr3, byte[][][] bArr4, byte[][][] bArr5, Treehash[][] treehashArr, Treehash[][] treehashArr2, Vector[] vectorArr, Vector[] vectorArr2, Vector[][] vectorArr3, Vector[][] vectorArr4, GMSSLeaf[] gMSSLeafArr, GMSSLeaf[] gMSSLeafArr2, GMSSLeaf[] gMSSLeafArr3, int[] iArr2, byte[][] bArr6, GMSSRootCalc[] gMSSRootCalcArr, byte[][] bArr7, GMSSRootSig[] gMSSRootSigArr, GMSSParameters gMSSParameters, GMSSDigestProvider gMSSDigestProvider) {
        ...
        this.f9876K = gMSSParameters.getK();
        ...
        if (bArr5 == null) {
            ...
            for (int i2 = 0; i2 < this.numLayer; i2++) {
                this.keep[i2] = (byte[][]) Array.newInstance((Class<?>) byte.class, (int) Math.floor(this.heightOfTrees[i2] / 2), this.mdLength);
            }
        } else {
            this.keep = bArr5;
        }
        ...
        if (gMSSRootCalcArr == null) {
            ...
            for (int i7 = 1; i6 < this.numLayer - i7; i7 = 1) {
                int i8 = i6 + 1;
                this.nextNextRoot[i6] = new GMSSRootCalc(this.heightOfTrees[i8], this.f9876K[i8], this.digestProvider);
                i6 = i8;
            }
        } else {
    ...
    private void computeAuthPaths(int i) {
        ...
        if (heightOfPhi == 0) {
            ...
            for (int i12 = 0; i12 < heightOfPhi; i12++) {
                if (i12 < i2) {
                    if (this.currentTreehash[i][i12].wasFinished()) {
                        System.arraycopy(this.currentTreehash[i][i12].getFirstNode(), 0, this.currentAuthPaths[i][i12], 0, this.mdLength);
                        this.currentTreehash[i][i12].destroy();
                    } else {
                ...
                if (i12 < i9 && i12 >= i2) {
                    int i13 = i12 - i2;
                    if (this.currentRetain[i][i13].size() > 0) {
                        System.arraycopy(this.currentRetain[i][i13].lastElement(), 0, this.currentAuthPaths[i][i12], 0, this.mdLength);
                        Vector[][] vectorArr = this.currentRetain;
                        vectorArr[i][i13].removeElementAt(vectorArr[i][i13].size() - 1);
                    }
                }
                if (i12 < i2 && ((1 << i12) * 3) + i3 < this.numLeafs[i]) {
                    this.currentTreehash[i][i12].initialize();
                }
            }
        }
        if (heightOfPhi < i9 && i7 == 0) {
    ...
    private int getMinTreehashIndex(int i) {
        int i2 = -1;
        for (int i3 = 0; i3 < this.heightOfTrees[i] - this.f9876K[i]; i3++) {
    ...
    private void nextTree(int i) {
        if (i > 0) {
            ...
            for (int i5 = 0; i5 < this.heightOfTrees[i] - this.f9876K[i]; i5++) {
            ...
            for (int i7 = 0; i7 < this.f9876K[i] - 1; i7++) {
    ...
    private void updateKey(int i) {
        ...
        if (i > 0) {
            ...
            int floor = (int) Math.floor((getNumLeafs(i) * 2) / (this.heightOfTrees[i3] - this.f9876K[i3]));
            int[] iArr = this.index;
            if (iArr[i] % floor == 1) {
