...
public class McElieceKobaraImaiCipher implements MessageEncryptor {
    ...
    private void initCipherDecrypt(McElieceCCA2PrivateKeyParameters mcElieceCCA2PrivateKeyParameters) {
        ...
        this.f9905n = mcElieceCCA2PrivateKeyParameters.getN();
        this.f9904k = mcElieceCCA2PrivateKeyParameters.getK();
        this.f9907t = mcElieceCCA2PrivateKeyParameters.getT();
    }

    private void initCipherEncrypt(McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters) {
        this.messDigest = Utils.getDigest(mcElieceCCA2PublicKeyParameters.getDigest());
        this.f9905n = mcElieceCCA2PublicKeyParameters.getN();
        this.f9904k = mcElieceCCA2PublicKeyParameters.getK();
        this.f9907t = mcElieceCCA2PublicKeyParameters.getT();
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (cipherParameters instanceof ParametersWithRandom) {
            ...
            this.f9906sr = parametersWithRandom.getRandom();
            this.key = (McElieceCCA2PublicKeyParameters) parametersWithRandom.getParameters();
        } else {
            this.f9906sr = CryptoServicesRegistrar.getSecureRandom();
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        ...
        GF2Vector[] decryptionPrimitive = McElieceCCA2Primitives.decryptionPrimitive((McElieceCCA2PrivateKeyParameters) this.key, GF2Vector.OS2VP(this.f9905n, bArr));
        byte[] encoded = decryptionPrimitive[0].getEncoded();
        GF2Vector gF2Vector = decryptionPrimitive[1];
        if (encoded.length > i2) {
            encoded = ByteUtils.subArray(encoded, 0, i2);
        }
        byte[] concatenate = ByteUtils.concatenate(ByteUtils.concatenate(bArr2, Conversions.decode(this.f9905n, this.f9907t, gF2Vector)), encoded);
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        ...
        int digestSize = this.messDigest.getDigestSize();
        int i = this.f9904k >> 3;
        int bitLength = (IntegerFunctions.binomial(this.f9905n, this.f9907t).bitLength() - 1) >> 3;
        ...
        byte[] encoded = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) this.key, GF2Vector.OS2VP(this.f9904k, bArr9), Conversions.encode(this.f9905n, this.f9907t, bArr8)).getEncoded();
