...
public class RainbowSigner implements MessageSigner {
    ...
    /* renamed from: cf */
    private ComputeInField f9938cf = new ComputeInField();
    RainbowKeyParameters key;
    private SecureRandom random;
    int signableDocumentLength;

    /* renamed from: x */
    private short[] f9939x;

    private short[] initSign(Layer[] layerArr, short[] sArr) {
        short[] sArr2 = new short[sArr.length];
        short[] multiplyMatrix = this.f9938cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA1(), this.f9938cf.addVect(((RainbowPrivateKeyParameters) this.key).getB1(), sArr));
        for (int i = 0; i < layerArr[0].getVi(); i++) {
            this.f9939x[i] = (short) this.random.nextInt();
            short[] sArr3 = this.f9939x;
    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        this.f9939x = new short[((RainbowPrivateKeyParameters) this.key).getInvA2().length];
        ...
        do {
            try {
                ...
                while (i2 < length) {
                    ...
                    short[] solveEquation = this.f9938cf.solveEquation(layers[i2].plugInVinegars(this.f9939x), sArr);
                    if (solveEquation == null) {
                        throw new Exception("LES is not solveable!");
                        break;
                    }
                    for (int i6 = 0; i6 < solveEquation.length; i6++) {
                        this.f9939x[layers[i2].getVi() + i6] = solveEquation[i6];
                    }
                    i2++;
                    i3 = i4;
                }
                short[] multiplyMatrix = this.f9938cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA2(), this.f9938cf.addVect(((RainbowPrivateKeyParameters) this.key).getB2(), this.f9939x));
