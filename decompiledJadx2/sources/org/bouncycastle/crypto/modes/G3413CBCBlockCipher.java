...
public class G3413CBCBlockCipher implements BlockCipher {
    ...
    private int decrypt(byte[] bArr, int i, byte[] bArr2, int i2) {
        byte[] MSB = GOST3413CipherUtil.MSB(this.f9503R, this.blockSize);
    ...
    private int encrypt(byte[] bArr, int i, byte[] bArr2, int i2) {
        byte[] sum = GOST3413CipherUtil.sum(GOST3413CipherUtil.copyFromInput(bArr, this.blockSize, i), GOST3413CipherUtil.MSB(this.f9503R, this.blockSize));
    ...
    private void generateR(byte[] bArr) {
        byte[] LSB = GOST3413CipherUtil.LSB(this.f9503R, this.f9504m - this.blockSize);
        System.arraycopy(LSB, 0, this.f9503R, 0, LSB.length);
        System.arraycopy(bArr, 0, this.f9503R, LSB.length, this.f9504m - LSB.length);
    }

    private void initArrays() {
        int i = this.f9504m;
        this.f9503R = new byte[i];
        this.R_init = new byte[i];
    }

    private void setupDefaultParams() {
        this.f9504m = this.blockSize;
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) throws IllegalArgumentException {
        ...
        if (!(cipherParameters instanceof ParametersWithIV)) {
            ...
            System.arraycopy(bArr, 0, this.f9503R, 0, bArr.length);
        ...
        this.f9504m = iv.length;
        initArrays();
        this.R_init = Arrays.clone(iv);
        byte[] bArr2 = this.R_init;
        System.arraycopy(bArr2, 0, this.f9503R, 0, bArr2.length);
