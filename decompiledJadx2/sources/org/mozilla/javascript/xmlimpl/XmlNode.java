...
public class XmlNode implements Serializable {
    ...
    public static XmlNode newElementWithText(XmlProcessor xmlProcessor, XmlNode xmlNode, QName qName, String str) {
        ...
        if (namespace == null || namespace.getUri().length() == 0) {
            createElementNS = newDocument.createElementNS(null, qName.getLocalName());
        } else {
            createElementNS = newDocument.createElementNS(namespace.getUri(), qName.qualify(node));
        }
        if (str != null) {
    ...
    public void removeNamespace(Namespace namespace) {
        if (namespace.m4196is(getNodeNamespace())) {
            return;
        }
        NamedNodeMap attributes = this.dom.getAttributes();
        for (int i = 0; i < attributes.getLength(); i++) {
            if (namespace.m4196is(createImpl(attributes.item(i)).getNodeNamespace())) {
                return;
            }
        }
    ...
    public static class Namespace implements Serializable {
        ...
        /* renamed from: is */
        boolean m4196is(Namespace namespace) {
            String str;
