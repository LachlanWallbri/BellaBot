...
public class ClientState {
    ...
    static {
        String name = ClientState.class.getName();
        CLASS_NAME = name;
        ...
        synchronized (this.quiesceLock) {
            ...
            synchronized (this.pingOutstandingLock) {
                ...
                if ((this.pingOutstanding == 0 && currentTimeMillis - this.lastInboundActivity >= this.keepAlive - 100) || currentTimeMillis - this.lastOutboundActivity >= this.keepAlive - 100) {
                    ...
                    long max = Math.max(1L, getKeepAlive() - (currentTimeMillis - this.lastOutboundActivity));
                    mqttToken = null;
                    j = max;
                }
            }
            log.fine(CLASS_NAME, "checkForActivity", "624", new Object[]{new Long(j)});
            this.pingSender.schedule(j);
            return mqttToken;
        }
    }
        ...
        synchronized (this.queueLock) {
            ...
            while (mqttWireMessage == null) {
                if ((this.pendingMessages.isEmpty() && this.pendingFlows.isEmpty()) || (this.pendingFlows.isEmpty() && this.actualInFlight >= this.maxInflight)) {
                    try {
                        log.fine(CLASS_NAME, TmpConstant.PROPERTY_IDENTIFIER_GET, "644");
                        this.queueLock.wait();
                        log.fine(CLASS_NAME, TmpConstant.PROPERTY_IDENTIFIER_GET, "647");
                    } catch (InterruptedException unused) {
                    }
                }
                if (!this.connected && (this.pendingFlows.isEmpty() || !(((MqttWireMessage) this.pendingFlows.elementAt(0)) instanceof MqttConnect))) {
                    log.fine(CLASS_NAME, TmpConstant.PROPERTY_IDENTIFIER_GET, "621");
                    return null;
                }
                if (!this.pendingFlows.isEmpty()) {
                    ...
                    if (mqttWireMessage instanceof MqttPubRel) {
                        ...
                        log.fine(CLASS_NAME, TmpConstant.PROPERTY_IDENTIFIER_GET, "617", new Object[]{new Integer(this.inFlightPubRels)});
                    }
                    checkQuiesceLock();
                    ...
                    if (this.actualInFlight < this.maxInflight) {
                        ...
                        this.actualInFlight++;
                        log.fine(CLASS_NAME, TmpConstant.PROPERTY_IDENTIFIER_GET, "623", new Object[]{new Integer(this.actualInFlight)});
                    } else {
                        log.fine(CLASS_NAME, TmpConstant.PROPERTY_IDENTIFIER_GET, "622");
                    }
                }
            }
            return mqttWireMessage;
