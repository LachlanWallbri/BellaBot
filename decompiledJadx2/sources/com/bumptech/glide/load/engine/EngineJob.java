...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ...
                next.executor.execute(new CallResourceReady(next.f1235cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ResourceCallbackAndExecutor next = it.next();
                next.executor.execute(new CallLoadFailed(next.f1235cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    public class CallLoadFailed implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f1233cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f1233cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f1233cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f1233cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f1233cb);
                    }
    ...
    public class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f1234cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f1234cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f1234cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f1234cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f1234cb);
                        EngineJob.this.removeCallback(this.f1234cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    public static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb */
        final ResourceCallback f1235cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f1235cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f1235cb.equals(((ResourceCallbackAndExecutor) obj).f1235cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f1235cb.hashCode();
        }
    }
