...
public class RPLidarA3Protocol {
    ...
    Capsule coverBytes(byte[] bArr) {
        ...
        capsule.syncCheckSumLow = bArr[0];
        capsule.syncCheckSumHigh = bArr[1];
    ...
    public class Node {
        ...
        public String toString() {
            return "origin node angle=" + this.angle + " distanceMm=" + this.distanceMm + " quality=" + ((int) this.quality) + " flag=" + ((int) this.flag);
        }
    }
    ...
    private int getCurrentAngleRaw(int i, int i2, ArrayList<Node> arrayList, int[] iArr) {
        ...
        while (i4 < 3) {
            ...
            node.angle = ((iArr2[i4] << 8) / 90) & 65535;
            node.distanceMm = iArr[i4];
            arrayList.add(node);
            i4++;
            i3 = i5;
        }
