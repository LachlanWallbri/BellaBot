...
public class LottieCompositionFactory {
    ...
    private static LottieResult<LottieComposition> fromJsonInputStreamSync(InputStream inputStream, String str, boolean z) {
        try {
            return fromJsonReaderSync(JsonReader.m81of(Okio.buffer(Okio.source(inputStream))), str);
        } finally {
            if (z) {
    ...
    public static LottieResult<LottieComposition> fromJsonStringSync(String str, String str2) {
        return fromJsonReaderSync(JsonReader.m81of(Okio.buffer(Okio.source(new ByteArrayInputStream(str.getBytes())))), str2);
    }
        ...
        try {
            ...
            while (nextEntry != null) {
                ...
                if (name.contains("__MACOSX")) {
                    ...
                    lottieComposition = fromJsonReaderSyncInternal(JsonReader.m81of(Okio.buffer(Okio.source(zipInputStream))), null, false).getValue();
                } else {
                    if (!name.contains(FileInfo.EXTEND_PNG) && !name.contains(".webp")) {
                        zipInputStream.closeEntry();
                    }
