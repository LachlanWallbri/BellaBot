...
public final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair m733of = EndpointPair.m733of(this, n, n2);
            Preconditions.checkArgument(incidentNodes.equals(m733of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, incidentNodes, m733of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m730of();
            }
            return DirectedNetworkConnections.m731of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m736of();
        }
        return UndirectedNetworkConnections.m737of();
    }
}
